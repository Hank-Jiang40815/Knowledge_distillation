...
def main():
    parser = argparse.ArgumentParser(description="DCCRN Model Simplified Inference")
    parser.add_argument("-C", "--config", default="config/base_config.toml", type=str, help="Config file path (.toml)")
    parser.add_argument("--model", type=str, help="Path to the model weights file (.pth)")
    parser.add_argument("--input", type=str, help="Path to input noisy audio file (for single-file mode)")
    parser.add_argument("--output", type=str, help="Path to output enhanced audio file (for single-file mode)")
    parser.add_argument("--batch_size", type=int, default=1, help="Batch size for inference")
    parser.add_argument("--max_files", type=int, default=None, help="Maximum number of files to process (for testing)")
    parser.add_argument("--disable_audio_save", action="store_true", help="Disable saving enhanced audio files")
    args = parser.parse_args()

    print(f"===== 简化版DCCRN推理脚本 - 启动时间: {time.strftime('%Y-%m-%d %H:%M:%S')} =====")
    ...
    model_path = args.model if args.model else config["path"]["pre_model"]
    output_path = os.path.join(base_path, "enhanced", "base")
    metrics_path = os.path.join(base_path, "metrics", "base")
